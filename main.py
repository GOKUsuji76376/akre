...
from NewFileName import Newfilename
from admin import Adminarun
...
async def start_handler(bot: Client, event: Message):
    ...
    await event.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("Bots Channel", url="https://t.me/akbots001")],
            ]
        )
    )
...
async def rename_handler(bot: Client, event: Message):
    ...
    if media and media.file_name:
        ...
        try:
            admin = [int(i) for i in Adminarun]
            print(admin)
            print(event.from_user.id)
            user_id_rename = event.from_user.id
            if user_id_rename in admin:
                print("here")
                ask_ = Newfilename(media.file_name)
            else:
                ask_: Message = await bot.listen(event.chat.id, timeout=300)

            if ask_["text"] and (ask_["text"][0] != "/"):
                ascii_ = ''.join([i if (i in string.digits or i in string.ascii_letters or i == " " or i == "@" or i == "-", i== "." ) else "" for i in ask_["text"].rsplit('.', 1)[0]])
...
async def callback_handlers(bot: Client, cb: CallbackQuery):
    if "closeMeh" in cb.data:
        ...
        if current_prefix is None:
            await cb.answer("Currently you didn't set a ny File Name Prefix!", show_alert=True)
            await cb.message.edit(
